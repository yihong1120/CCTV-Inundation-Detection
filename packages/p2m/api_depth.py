...
import tflearn
import numpy as np
from tensorflow.compat.v1 import variable_scope, add, concat, get_collection, GraphKeys, train, app, expand_dims, squeeze
#tf.disable_v2_behavior()
#from tensorflow.keras.layers import *
import os
import sys
sys.path.append(os.path.join(os.getcwd(),'Mask_RCNN','p2m'))
from layers_depth import Layer, GraphConvolution, GraphPooling, GraphProjection
#import layers
...
class Model(object):
    ...
    def build(self):
        ...
        for idx,layer in enumerate(self.layers):
            ...
            if idx in eltwise:
                hidden = add(hidden, self.activations[-2]) * 0.5
            if idx in concat_depth:
                hidden = concat([hidden, self.activations[-2]], 1)
        ...
        variables = get_collection(GraphKeys.GLOBAL_VARIABLES, scope=self.name)
    ...
    def save(self, sess=None):
        ...
        saver = train.Saver(self.vars)
        save_path = saver.save(sess, "./Mask_RCNN/Data/checkpoint/%s.ckpt" % self.name)
    ...
    def load(self, sess=None):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        saver = train.Saver(self.vars)
        save_path = "./Mask_RCNN/Data/checkpoint/%s.ckpt" % self.name
...
class GCN(Model):
    ...
    def __init__(self, placeholders, **kwargs):
        ...
        self.inputs = placeholders['features_depth']
        self.placeholders = placeholders

        self.optimizer = train.AdamOptimizer(learning_rate=0)
    ...
    def _build(self):
        ...
        self.layers.append(GraphConvolution(input_dim=963,
                                            output_dim=256,
                                            gcn_block_id=1,
        ...
        self.layers.append(GraphConvolution(input_dim=963+256,
                                            output_dim=256,
                                            gcn_block_id=2,
                                            placeholders=self.placeholders, logging=self.logging))
        for _ in range(12):
        ...
        self.layers.append(GraphConvolution(input_dim=256,
                                            ...
                                            gcn_block_id=3,
                                            placeholders=self.placeholders, logging=self.logging))
        self.layers.append(GraphConvolution(input_dim=int(256/2),
                                            output_dim=3,
                                            act=lambda x: x,
                                            gcn_block_id=3,
                                            placeholders=self.placeholders, logging=self.logging))
    ...
    def build_cnn18(self):
        x=self.placeholders['img_inp_depth']
        x=expand_dims(x, 0)
#224 224
...
#updata image feature
        self.placeholders.update({'img_feat': [squeeze(x2), squeeze(x3), squeeze(x4), squeeze(x5)]})
